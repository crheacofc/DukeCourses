...
def assemble(con_mat,Coord_mat,el_type='Q4',P_type='scalar'):
    ...
    for e in range(len(con_mat)):
        ...
        Coord_mat_el_fix = np.zeros((2, num_node)) #top right - top left - bottom left - bottom right
        Coord_mat_el_fix[:,0] = Coord_mat_el[:,2]
        Coord_mat_el_fix[:, 1] = Coord_mat_el[:, 3]
        Coord_mat_el_fix[:, 2] = Coord_mat_el[:, 0]
        Coord_mat_el_fix[:, 3] = Coord_mat_el[:, 1]
        K_e = Dif_basic_K(el_type, Coords=Coord_mat_el_fix.T)
        print(K_e)

        F_e = Dif_basic_F(el_type, lambda x, y: x, Coord_mat_el_fix.T)
        ...
        for i in range(num_dof_el):
            ...
            # end j loop
            F[dof_1] += F_e[0][i] #first element is nothing and second is dof
        #end i loop
    #end elemental loop
