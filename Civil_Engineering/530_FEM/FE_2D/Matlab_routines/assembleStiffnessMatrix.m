...
% homework
    num_node = 4;
    [num_nodes_total,num_dim] = size(NodalCoord); 
    num_dof = num_node*num_dim; %per element
    %num_dof_total = 12;%% 2*sizeof(NodalCoord)
    
    num_dof_total = num_nodes_total*num_dim;
    A = zeros(length(NodalCoord),num_dof); %create assembly matrix which is num_elements x num_dof
    for e = 1:2 %each element
         for i =1:4 %all the columns of a row
             for j = 1:num_dim %num of dof per node
                % disp(j)
                  if (j==1)
                      A(e,(i-1)*num_dim+j) = num_dim*Connectivity(e,i)-1 ; %x Dof
                    %disp(j)
                  else
                        A(e,(i-1)*num_dim+j) = 2*Connectivity(e,i); %y Dof
                  end
             end %end j
         end %end i
         %disp(e)
