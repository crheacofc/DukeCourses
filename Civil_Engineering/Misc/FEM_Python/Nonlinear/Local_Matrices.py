...
def ke(xIP,C,D,el_epsilon,pt,weights,thickness,E,Ke):
    for i in range(len(xIP)):
        for j in range(len(xIP)):
            gradN, detJ = E.G_map(xIP[i], xIP[j], C)
            print("Coordinates are : \n",C)
            print("Length of Coordinate Matrix is: ",len(C))
            B = get_B_plane_stress(gradN)
            Ke += np.matmul(np.matmul(np.transpose(B),D(el_epsilon, pt)), B) * detJ * weights[i] * weights[j] * thickness
