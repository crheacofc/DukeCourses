...
def element_assembly(mutex,A,num_dof,e,NodalCoord,Connectivity,D,D_prime,thickness,K,Fsig,dk_du,epsilon,sigma):
    mutex.acquire()
    try:
...
def assembleNL(NodalCoord,Connectivity,D,D_prime,thickness,epsilon,sigma,el_type='Q4',P_type='vector',Prob='ES'):
    num_node = 4
    ...
    #    element_assembly(A,num_dof,i, NodalCoord,Connectivity,D,D_prime,thickness,K,Fsig,dk_du,epsilon,sigma)
    #Parallel(n_jobs=1)(delayed(element_assembly)(A,num_dof,i, NodalCoord,Connectivity,D,D_prime,thickness,K,Fsig,dk_du,epsilon,sigma) for i in range(len(Connectivity)))
    mutex = Lock()
    t = Thread(target=element_assembly, args=(mutex,A,num_dof,i, NodalCoord,Connectivity,D,D_prime,thickness,K,Fsig,dk_du,epsilon,sigma))
    t.start()
    #end elemental loop
